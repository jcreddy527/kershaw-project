   ...
   it('should retrieve document type list',
        inject([DocumentService, MockBackend], fakeAsync((documentService: DocumentService, mockBackend: MockBackend) => {
            ...
            mockBackend.connections.subscribe(c => {
                expect(c.request.url).toBe('http://localhost:8080/documentType/withFields');
    ...
    it('should be create document type',inject([DocumentService] , (service: DocumentService) => {

        let myfixture = TestBed.createComponent(HomeComponent);
        myfixture.detectChanges();        
        var button = <HTMLElement>myfixture.nativeElement.getElementsById('document_id');
        button.click();
        
    expect(component.nameEnabled).toBe(true);
  }));
